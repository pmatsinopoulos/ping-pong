function canvasApp() {
	...
	//============ CLASS DECLARATIONS ==========================================================================================================
	// 
	function Table(color, x, y, w, h) {
		this.color = color;
		this.position = new Position(x, y);
		this.size = new Size(w, h);
	} // Table
	
	function Ball(color, x, y, angle, radius) {
		this.color = color;
		this.position = new Position(x, y);
		this.angle = angle;
		this.radius = radius;
	...
	function BallDirection(radians, xunits, yunits) {
	...
	function Application(options) {
		...
		var ball = new Ball(options['ballColor'], canvas.width / 2, canvas.height / 2, options['ballStartingAngle'], options['ballRadius']);
		// This will hold the mouse position at runtime
		var mousePosition = new Position(0, 0);	
		
		// The lest and right tables
		var tables = new Array();
		tables.push(new Table(options['leftTableColor'], 0 + options['tableLeftOffset'], 0, options['tableWidth'], options['tableHeight']));
		tables.push(new Table(options['rightTableColor'], canvas.width - options['tableRightOffset'] - options['tableWidth'], 0, options['tableWidth'], options['tableHeight']));			    				    
		...
		var calculateBallDirection = function() {
			...
			return new BallDirection(radians,
		...
		var updateBall = function() {
		   ...
		   if (ball.position.x + ball.radius < tables[0].position.x || ball.position.x - ball.radius > tables[1].position.x + tables[1].size.width) {
...
// call the canvasApp function when document is ready
