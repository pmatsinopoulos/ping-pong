...
options: 
 ...
 -- ballStartingAngle
 */
PingPong.Application = function(options) {					
	...
	var updateBall = function() {
	   ...
	   if (!gameOver) {
		   // if ! gameOver then check whether it is gameOver or whether we have to bounce the ball.
	   	   if (ball.position.x + ball.radius < tables[0].position.x + tables[0].size.width || ball.position.x - ball.radius > tables[1].position.x) {
				...
				gameOver = true; 
				// we will leave 2 seconds before stop tracking ball.
				setTimeout(function() {clearInterval(updateBallTimerId);}, 2000);
...
// ------------------------------------------------------------------------------------------------
// end of PingPong Class Declarations
// ------------------------------------------------------------------------------------------------
	
// ================================================================================================
// PingPong Static Functions
// ================================================================================================
PingPong.canvasApp = function() {
	...
	if (!Modernizr.canvas) {
        dbg.log('Canvas is not supported');
		return false;
	} else {
		dbg.log('Canvas is supported');
	}
	
	var app = new PingPong.Application({canvasElementId:    'canvasOne', 
							            canvasWidth:        500,         
										canvasHeight:       500,         
